...
db.serialize(() => {
  ...
  // Adiciona a coluna 'badges' se ela ainda não existir (caso o banco já exista)
  db.get("PRAGMA table_info(user_profiles)", (err, columns) => {
...
function getUserProfile(userId) {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT xp, level, bio, background, badges FROM user_profiles WHERE user_id = ?',
      [userId],
      (err, row) => {
        if (err) return reject(err);
        if (row) return resolve(row);
...
// Funções de leaderboard XP
function getXPLeaderboard(guildId) {
  return new Promise((resolve, reject) => {
    db.all(
      'SELECT user_id, xp, level FROM xp_leaderboard WHERE guild_id = ? ORDER BY xp DESC',
      [guildId],
      (err, rows) => {
        if (err) return reject(err);
        resolve(rows);
